...
%  - Each teacher can have at most one exam per week					done
%  - Each student can have at most 2 exams per week					done
...
coursesTeacher(T, C):- course(C, T) .
examsStudent(S, C):- course(C), student(S, E), member(C, E).
...
eachCExactly3HoursAtWeek:- course(C), findall(courseWeek-C-W, week(W), Lits), exactly(1, Lits), false.
eachCExactly3HoursAtWeek.

eachWExactly5Exams:- week(W), findall(courseWeek-C-W, course(C), Lits), exactly(5, Lits), false.
eachWExactly5Exams.

eachTAMOExamWeek:- teacher(T), week(W), findall(courseWeek-C-W, coursesTeacher(T, C), Lits), atMost(1, Lits), false.
eachTAMOExamWeek.

eachSAM2ExamWeek:- student(S), week(W), findall(courseWeek-C-W, examsStudent(S, C), Lits), atMost(2, Lits), false.
eachSAM2ExamWeek.

defineLastS:- student(S), examsStudent(S, C), writeClause([last-S, \+courseWeek-C-3]), fail.
defineLastS.

defineNoLastS(K):- findall(\+last-S, student(S), Lits), K1 is 20 - K, atLeast(K1, Lits), fail.
defineNoLastS(_).
...
%    write('Solution found with cost '), write(K), write('!'), nl, displaySol(M), nl.
    %No se porque pero no me ha salido hacer el bucle de optimizaciones... Pero el optimo son
    %9 estudiantes como muestra la solución. Muchas gracias, alejandro.
