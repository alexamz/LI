...
doubleConstraint(R, T, [home-T-R, home-T-R1]):- R > 1, R1 is R - 1, noDoublesRestriction(R).
...
writeClauses:-
    defineHome,
    eachPairTRexactlyOneRound,
    eachPairTSexactlyOneRound,
    defineDouble,
    %% eachTAtmostDouble,
    defineNoDoubles,
    defineTVMatch,
...
defineDouble:- team(T), roundRestrictionDouble(R), doubleConstraint(R, T, Lits), expressAnd(double-T-R, Lits), fail.                %negate(double-T-R, NVar), expressOr(NVar, Lits), fail.
defineDouble.

eachPairTSexactlyOneRound:- team(T), team(S), T \= S, 
                            findall(match-T-S-R, round(R), Home),
                            findall(match-S-T-R, round(R), Away),
                            union(Home, Away, Lits),
                            exactly(1, Lits),
                            fail.
eachPairTSexactlyOneRound.           

eachPairTRexactlyOneRound:- team(T), round(R), 
                            findall(match-T-S-R, matchOfT(T, T-S), Home),
                            findall(match-S-T-R, matchOfT(T, S-T), Away),
                            union(Home, Away, Lits),
                            exactly(1, Lits),
                            fail.
eachPairTRexactlyOneRound.  
...
eachTAtmostDouble:- team(T), findall(double-T-R, roundRestrictionDouble(R), Lits), atMost(1, Lits), fail.
eachTAtmostDouble.

defineNoDoubles:- team(T), round(R), expressOr(\+double-T-R, [home-T-2, home-T-1]), fail. % expressOr(\+double-T-R, [home-T-15, home-T-14]), fail.
defineNoDoubles.
