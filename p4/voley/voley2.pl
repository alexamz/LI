...
noDoublesRestriction(R):- noDoubles(L), \+ member(R, L).
...
defineDouble :- team(T), round(R), R>1, R1 is R-1, expressAnd(double-T-R, [home-T-R1, home-T-R]), fail.
defineDouble.
 
eachTSexactlyoneR :- teams(L), nth1(Ti,L,T), nth1(Si,L,S), Ti<Si,
    findall(match-P-Q-R, (matchOfTS(T,S,P-Q), round(R)), Lits), exactly(1, Lits), fail.
eachTSexactlyoneR.
 
eachRTexactlyoneS :- round(R), team(T), findall(match-P-Q-R, matchOfT(T,P-Q), Lits), exactly(1, Lits), fail.
eachRTexactlyoneS.
