...
eachHatMostOneCAtRoom:- room(R), course(C1), course(C2), C1 \= C2, day(D), hour(H), 
    atMost(3, [courseHour-C1-D-H, courseRoom-C1-R, courseHour-C2-D-H,  courseRoom-C2-R]), fail.
eachHatMostOneCAtRoom.

eachSatMost3HoursAtDay:- student(S), student(S, L), day(D), findall(courseHour-C-D-H, (select(C, L, _), hour(H)), Lits), 
    atMost(3, Lits), fail.
eachSatMost3HoursAtDay.

eachSatMost1courseAtHour:- student(S), student(S, L), day(D), hour(H), findall(courseHour-C-D-H, ( select(C, L, _)), Lits), 
    atMost(1, Lits), fail.
eachSatMost1courseAtHour.
...
defineNoLateD(N):- findall(\+late-D, day(D), Lits), atLeast(N, Lits), fail. % ([\+late-D]), fail.
defineNoLateD(_).
...
main:-  initClauseGeneration,
	...
	shell('time picosat -v -o model infile.cnf', Result),  % if sat: Result=10; if unsat: Result=20.
	treatResult(Result),!.
	
main(ND):-  symbolicOutput(1), !, writeClauses(ND), halt.   % print the clauses in symbolic form and halt
main(ND):-  initClauseGeneration,
        tell(clauses), writeClauses(ND), told,          % generate the (numeric) SAT clauses and call the solver
	tell(header),  writeHeader,  told,
	numVars(N), numClauses(C),
	write('Generated '), write(C), write(' clauses over '), write(N), write(' variables. '),nl,
	shell('cat header clauses > infile.cnf',_),
	write('Calling solver....'), nl, 
	shell('time picosat -v -o model infile.cnf', Result),  % if sat: Result=10; if unsat: Result=20.
	treatResult(Result, ND),!.
	
treatResult(10):- write('Solution found: '), nl, see(model), symbolicModel(M), seen, displaySol(M), nl,nl, main(1), halt. %main(1).
treatResult(20):- write('Unsatisfiable'), nl, halt.

treatResult(20, _):- write('Unsatisfiable'), nl, halt.
treatResult(10, N):- write('Solution found: '), nl, see(model), symbolicModel(M), seen, displaySol(M), nl,nl, N1 is N + 1, main(N1).
